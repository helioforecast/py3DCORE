...
def extract_fitvars(fittingstate_values):
    ...
    if fittingstate_values[17] == 'abc-smc':
        ...
        base_fitter.initialize(t_launch, py3dcore.ToroidalModel, model_kwargs)
        base_fitter.add_observer(fitobserver, t_fit, t_s, t_e)
        
    njobs = fittingstate_values[18]
    
    if fittingstate_values[19][0] == 'multiprocessing':
    ...
    outputfile = fittingstate_values[22]['id'][0]+ fittingstate_values[16][0]
    
    current_time = str(int(time.time()))
    
    if isinstance(outputfile, str):
        outputfilecode = "output/" + outputfile + "_" + current_time + "/"
    elif isinstance(outputfile, list) and len(outputfile) == 1 and isinstance(outputfile[0], str):
        outputfilecode = "output/" + outputfile[0] + "_" + current_time + "/"
